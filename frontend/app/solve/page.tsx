...
import { solveImage, type SolveResponse, type CellData } from "@/lib/api";
      ...
      {TABS.map(({ id, label }) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(id)}
            className="btn-press relative z-10 rounded-[var(--r-pill)] px-4 py-1.5 text-caption transition-all duration-200"
            style={{
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <button
          onClick={onCopy}
          className="btn-press flex items-center gap-2 rounded-[var(--r-sm)] px-3 py-2 text-caption transition-all"
          style={{
...
export default function SolvePage() {
  ...
  async function handleSolve() {
    ...
    try {
      const data = await solveImage(file, abortRef.current.signal);
      setResult(data);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          gridTemplateColumns: result ? "minmax(340px,420px) 1fr" : "minmax(340px,480px)",
          alignItems: "start",
        }}
      >
        {/* ── LEFT PANEL ──────────────────────────────────────────── */}
