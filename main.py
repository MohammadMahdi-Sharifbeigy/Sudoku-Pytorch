...
# ==========================================
    ...
    st.markdown("Convert trained model to ONNX and TorchScript; compare CPU latency and sizes.")

    cpu_device  = torch.device('cpu')
    opt_model_type = st.radio("Model to optimize:", ["Single-task (DigitCNN)", "Multi-task (MultiTaskDigitCNN)"])

    if opt_model_type.startswith("Multi"):
        opt_path = 'models/best_model_multitask.pt'
        if not os.path.exists(opt_path):
            st.error("Multi-task model not found. Train with the multi-task toggle first.")
        else:
            base_model = MultiTaskDigitCNN(num_digit_classes=10, num_lang_classes=2)
            base_model.load_state_dict(torch.load(opt_path, map_location=cpu_device))
            base_model.to(cpu_device).eval()
            st.info("Loaded MultiTaskDigitCNN.")

            if st.button("Run Optimization & Benchmark"):
                with st.spinner("Optimizing…"):
                    results, verif = run_optimization_and_benchmark_multitask(base_model, opt_path, cpu_device)

                if verif.get('error'):
                    st.warning(f"ONNX Runtime not installed — skipping ONNX benchmark. `pip install onnxruntime`")
                elif not verif['digit_match'] or not verif['lang_match']:
                    st.error(f"ONNX output mismatch! digit_match={verif['digit_match']} "
                             f"lang_match={verif['lang_match']}")
                else:
                    st.success("ONNX verified: both digit and language heads match PyTorch outputs.")

                st.dataframe(pd.DataFrame(results), width='stretch')
                st.markdown("### Deployment")
                st.markdown(
                    "- **TorchScript**: `torch.jit.load('models/best_model_multitask.ts')`\n"
                    "- **ONNX**: `onnxruntime.InferenceSession('models/best_model_multitask.onnx')` "
                    "— outputs are `digit_output` and `lang_output`."
                )
    else:
            ...
            if st.button("Run Optimization & Benchmark"):
                with st.spinner("Optimizing and benchmarking..."):
                    ...
                    if results[-1]["Inference Latency (ms)"] == "N/A":
                        st.warning("ONNX Runtime not installed. Install with `pip install onnxruntime`.")

                    st.success("Optimization and Benchmarking complete!")
                    st.dataframe(pd.DataFrame(results), width='stretch')
                    st.markdown("### Deployment Instructions")
                    st.markdown(
                        "- **TorchScript**: `torch.jit.load('models/best_model.ts')`\n"
                        "- **ONNX**: `onnxruntime.InferenceSession('models/best_model.onnx')`"
                    )