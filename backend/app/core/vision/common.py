"""
common.py – Detector-agnostic Sudoku vision helpers shared by all detectors.
...
def plot_cell_images_in_grid(cells: list[dict], scale: int = 4) -> np.ndarray:
    """Return the extracted 28x28 cell images as one 9x9 display grid."""
    if len(cells) != 81:
        raise ValueError(f"Expected 81 cells, got {len(cells)}")

    tile_size = 28 * scale
    gap = max(1, scale)
    grid_size = 9 * tile_size + 10 * gap
    canvas = np.full((grid_size, grid_size), 30, dtype=np.uint8)

    for idx, cell in enumerate(cells):
        row, col = divmod(idx, 9)
        tile = cv2.resize(cell["img"], (tile_size, tile_size), interpolation=cv2.INTER_NEAREST)
