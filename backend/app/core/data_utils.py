...
def __convert_to_one_hot(vector: np.ndarray, num_classes: int) -> np.ndarray:
    result = np.zeros(shape=[len(vector), num_classes])
    result[np.arange(len(vector)), vector] = 1
    return result


def __resize_image(src_image: np.ndarray, dst_image_height: int, dst_image_width: int) -> np.ndarray:
...
def read_hoda_cdb(file_name: str) -> tuple[list[np.ndarray], list[int]]:
    with open(file_name, 'rb') as binary_file:
        ...
        imgType = struct.unpack_from('B', data, offset)[0]; offset += 1
...
def generate_empty_cells(num_samples: int = 3000) -> tuple[torch.Tensor, ...]:
...
def get_font_image_dict(data_path: str, excluded_names: Optional[list[str]] = None) -> dict[int, np.ndarray]:
    ...
    img_dict: dict[int, np.ndarray] = {i: [] for i in range(1, 10)}  # type: ignore[assignment]
    ...
    for digit_class in range(1, 10):
        for fpath in digit_image_filepaths[digit_class - 1]:
            try:
                ...
                img_dict[digit_class].append(resized)  # type: ignore[attr-defined]
            except Exception:
                pass
...
def load_font_image_arrays(image_dict: dict[int, np.ndarray]) -> tuple[torch.Tensor, ...]:
    ...
    return (torch.from_numpy(x_train).float(), torch.from_numpy(x_val).float(), torch.from_numpy(x_test).float(),
            torch.from_numpy(y_train.flatten()).long(), torch.from_numpy(y_val.flatten()).long(),
...
def get_dataloaders(data_path: str, batch_size: int = 128) -> tuple[DataLoader, DataLoader, DataLoader]:  # type: ignore[type-arg]
    """Loader 1: MNIST + Fonts + Empty Cells"""
    x_tr_m, x_v_m, x_te_m, y_tr_m, y_v_m, y_te_m = load_mnist_images(data_path)
...
def get_dataloaders_mnist_hoda(data_path: str, batch_size: int = 128) -> tuple[DataLoader, DataLoader, DataLoader]:  # type: ignore[type-arg]
    ...
    if x_tr_h is not None:
        x_train_list.append(x_tr_h); y_train_list.append(y_tr_h)  # type: ignore[arg-type]
        x_val_list.append(x_v_h); y_val_list.append(y_v_h)  # type: ignore[arg-type]
        x_test_list.append(x_te_h); y_test_list.append(y_te_h)  # type: ignore[arg-type]
...
def get_dataloaders_all(data_path: str, batch_size: int = 128) -> tuple[DataLoader, DataLoader, DataLoader]:  # type: ignore[type-arg]
