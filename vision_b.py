"""
vision_b.py – Existing contour pipeline + HoughLinesP post-warp refinement.

Pipeline:
  Grid detection  : multi-param adaptive threshold → contours → NMS → perspective warp
  Hough refine    : HoughLinesP on warped image → exact 10H + 10V line positions
  Cell extraction : slice cells from Hough line bounds (primary)
                    fallback: contour-based → KMeans → deterministic slice
"""
...
def get_valid_cells_from_image(img, grid_size=576):
    """
    Full pipeline: detect grid → warp → Hough-refine → slice cells.

    Primary path  : existing contour detection → warp → HoughLinesP cell bounds
    Fallback 1    : contour-based cell detection → KMeans layout recovery
    Fallback 2    : deterministic 9×9 slice (original vision.py behaviour)
    """
    ...
    # ── Primary: Hough refinement on each candidate ───────────────────────────
    for i, grid_image in enumerate(warped_images):
        refined = refine_corners_hough_b(grid_image)
        if refined is not None:
            h_ys, v_xs = refined
            warped_clean = remove_grid_lines_hough_b(grid_image)
            cells = slice_cells_from_lines_b(warped_clean, h_ys, v_xs)
            digit_count = sum(c['contains_digit'] for c in cells)
            if digit_count > 0:
                return cells, M_matrices[i], grid_image

    # ── Fallback 1: existing contour cell detection ────────────────────────────
    best = None
    for i, grid_image in enumerate(warped_images):
    ...
    # ── Fallback 2: square-warp + deterministic slice ─────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Hough-based refinement (post-warp)
# ─────────────────────────────────────────────────────────────────────────────

def _cluster_positions_b(positions: list[float], gap: int = 15) -> np.ndarray:
...
def _preprocess_for_hough_b(warped_bgr: np.ndarray) -> np.ndarray:
...
def refine_corners_hough_b(warped_bgr: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
    """HoughLinesP → (h_ys, v_xs) sorted 10-element arrays, or None."""
    h, w = warped_bgr.shape[:2]
    binary = _preprocess_for_hough_b(warped_bgr)
...
def remove_grid_lines_hough_b(warped_bgr: np.ndarray) -> np.ndarray:
    """Return grayscale image with grid lines painted black."""
    h, w = warped_bgr.shape[:2]
    binary = _preprocess_for_hough_b(warped_bgr)
...
def slice_cells_from_lines_b(
    warped_clean: np.ndarray,
    h_ys: np.ndarray,
    v_xs: np.ndarray,
) -> list[dict]:
    """Slice 81 cell dicts from cleaned gray image using Hough line positions."""
    H, W = warped_clean.shape[:2]
    cells = []
    for r in range(9):
        for c in range(9):
            y0, y1 = max(0, int(h_ys[r])),   min(H, int(h_ys[r + 1]))
            x0, x1 = max(0, int(v_xs[c])),   min(W, int(v_xs[c + 1]))
